...
import static frmw.TestSupport.POSTGRESQL;
...
public class PostgreSQLTest {
	...
	public void addMonths() {
		...
		String sql = f.sql(POSTGRESQL);
		assertEquals("(\"col1\" + cast('5 months' as interval))", sql);
	}
	...
	public void random() {
		...
		String sql = f.sql(POSTGRESQL);
		assertEquals("(random() * (100 - 1) + 1)", sql);
	}
	...
	public void index() {
		...
		String sql = f.sql(POSTGRESQL);
		assertEquals("position('data' in \"col1\")", sql);
	}
	...
	public void zScoreComplexFormulaTest() {
		...
		f.windowParameters().get(0).partition(f2.sql(POSTGRESQL)).order("\"col_date\"", Order.DESC);
		f.windowParameters().get(1).partition(f2.sql(POSTGRESQL)).order("\"col_date\"", Order.DESC);
		...
		//System.out.println(f.sql(POSTGRESQL));
		assertEquals(expected, f.sql(POSTGRESQL));
	}
